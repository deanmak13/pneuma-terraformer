...
# pulled in by the first install (e.g. anyio's `typing_extensions`) into
# /install on the second command, since pip considered them "already
# satisfied" globally. The runtime stage only COPYs /install, so those
# skipped packages were never in the final image — first surfaced as a
# `ModuleNotFoundError: No module named 'typing_extensions'` crash-loop
# on first-ever boot (2026-07-17). A single combined install resolves the
# full dependency graph together and lands every transitive package in
# /install exactly once — the fix is the build mechanism, not pinning
# the one package that happened to go missing this time.
...
RUN pip install --no-cache-dir --prefix=/install /tmp/proto-wheels/pneuma_proto-*.whl .
